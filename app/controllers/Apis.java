    ...
    public Result totalesEPporPeriodo(Http.Request request, String desdeAAMMDD, String hastaAAMMDD, Double uf, Double usd, Double eur) {
        ...
        if (!baseDato.equals("0")) {
            ...
            try (Connection con = dbRead.getConnection()) {
                ...
                Map<Long, Long> mapBodConStockSoloArr = Inventarios.mapBodegasVigConStockSoloArr(con, s.baseDato);
                ...
                return ok("{\"datos\":" + rsJson + ",\"desde\":\"" + desdeAAMMDD + "\",\"hasta\":\"" + hastaAAMMDD + "\",\"empresa\":\"" + baseDato.substring(4) + "\"}").as("application/json");
            } catch (SQLException e) {
                e.printStackTrace();
                return ok("{\"ERROR\":\"contactar a INQSOL\"}").as("application/json");
            }
    ...
    public Result matrizInventarioPorCoti(Http.Request request, String fechaCorte) {
        ...
        if (!baseDato.equals("0")) {
            try (Connection con = dbRead.getConnection()) {
                ...
                con.close();
                return ok("{\"datos\":" + rsJson + ",\"fechaCorte\":\"" + fechaCorte + "\",\"empresa\":\"" + baseDato.substring(4) + "\"}").as("application/json");
            } catch (SQLException e) {
                e.printStackTrace();
                return ok("{\"ERROR\":\"contactar a INQSOL\"}").as("application/json");
            }
    ...
    public Result consolidadoPorGrupoMeses(Http.Request request, String fechaCorte, Long cantMeses) {
        ...
        if (!baseDato.equals("0")) {
            try (Connection con = dbRead.getConnection()) {
                ...
                con.close();
                return ok("{\"datos\":" + rsJson + ",\"desde\":\"" + inicioFecha.getFechaStrAAMMDD() + "\",\"hasta\":\"" + finFecha.getFechaStrAAMMDD() + "\",\"empresa\":\"" + baseDato.substring(4) + "\"}").as("application/json");
            } catch (SQLException e) {
                e.printStackTrace();
                return ok("{\"ERROR\":\"contactar a INQSOL\"}").as("application/json");
            }
    ...
    public Result estadoBodegasJson(Http.Request request) {
        ...
        if (!baseDato.equals("0")) {
            try (Connection con = dbRead.getConnection()) {
                ...
                String rsJson = Json.toJson(rs).toString();
                con.close();
                return ok("{\"datos\":" + rsJson + ",\"empresa\":\"" + baseDato.substring(4) + "\"}").as("application/json");
            } catch (SQLException e) {
                e.printStackTrace();
                return ok("{\"ERROR\":\"contactar a INQSOL\"}").as("application/json");
            }
