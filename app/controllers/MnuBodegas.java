    ...
    public Result bodegaCrear(Http.Request request) {
    	...
    	if(s.userName!=null && s.id_usuario!=null && s.id_tipoUsuario!=null && s.baseDato!=null && s.id_sucursal!=null && s.porProyecto!=null) {
    		...
    		try {
    			...
    			con.close();
    			return ok(bodegaCrear.render(mapeoDiccionario,mapeoPermiso,userMnu,listClientes,listProyectos,listTipoBodega,listRegiones,listSucursal,sucursal, s.aplicaPorSucursal));
        	} catch (SQLException e) {
    			e.printStackTrace();
    		}
    		return ok(mensajes.render("/",msgError));
