...
public class MnuCotizar extends Controller {
    ...
    public Result actualizaListaCotiAjax(Http.Request request) {
		...
		if (form.hasErrors()) {
			...
			try (Connection con = dbRead.getConnection()){
				List<List<String>> listCotizacion = Cotizacion.listCotiAllLimt(con, s.baseDato, s.aplicaPorSucursal, s.id_sucursal, 6000);
				String vistaHtml =
    ...
    public Result cotizaModifica(Http.Request request) {
		...
		if (form.hasErrors()) {
			...
			try (Connection con = dbRead.getConnection()){
				...
				List<Cliente> listClientes = Cliente.allsoloVigentes(con, s.baseDato);
				List<Proyecto> listProyectos = Proyecto.all(con, s.baseDato);
				List<List<String>> lista = FormCotiza.detalleCotizacion(con, s.baseDato, mapeoPermiso, mapeoDiccionario, id_bodegaEmpresa, id_cotizacion, s.id_sucursal, "1", "0", null);
				String numDecParaTot = "0";
				if(lista.size()>0) {
