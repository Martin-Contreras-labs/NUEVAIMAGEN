		...
		if (form.hasErrors()) {
			...
			try (Connection con = dbRead.getConnection()) {
				...
				listServicios.forEach(x -> {
					...
					String precioAdicional = DecimalFormato.formato(x.getPrecioAdicional(), x.getNroDecimal());
					List<String> aux = new ArrayList<String>();
					aux.add(x.getNombreClase());
					aux.add(x.getCodigo());
					aux.add(x.getNombre());
					aux.add(x.getNombreUnidad());
					aux.add(x.getNickMoneda());
					aux.add(precio);
					aux.add(x.getFecha());
					aux.add(x.getNroDecimal().toString());
					aux.add(x.getId().toString());
					aux.add(cantMinimo);
					aux.add(precioAdicional);
					listado.add(aux);
				});
	...
	public Result ajustesEpListadoOdo(Http.Request request) {
		...
		if (form.hasErrors()) {
			...
			try (Connection con = dbRead.getConnection()){
				...
				List<AjustesEpOdo> lista = AjustesEpOdo.allPorBodegaLimitFecha(con, s.baseDato, id_bodega, s.aplicaPorSucursal, s.id_sucursal, limitFecha.getFechaStrAAMMDD());
