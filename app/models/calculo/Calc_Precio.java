...
public class Calc_Precio {
		...
		try{
			...
			if(listaInBodega.trim().length()>0) {
				String query = String.format(" select "
						...
						+ " equipo.id_grupo "
						+ " from `%s`.listaPrecio "
						+ " left join `%s`.equipo on equipo.id = listaPrecio.id_equipo "
						+ " where id_bodegaEmpresa in ("+listaInBodega+");",db,db);
				try (PreparedStatement smt = con.prepareStatement(query)) {
					try (ResultSet rs = smt.executeQuery()) {
						while (rs.next()) {
							...
							if (id_unidadTiempo == 0) id_unidadTiempo = (long) 1;
					...
					String className = Calc_Precio.class.getSimpleName();
	...
	public static Map<Long,Double> mapDctoBodega (Connection con, String db){
		...
		String query = String.format(" select id, tasaDescto from `%s`.bodegaEmpresa where tasaDescto > 0;",db);
		try (PreparedStatement smt = con.prepareStatement(query)){
			try(ResultSet rs = smt.executeQuery()) {
				while (rs.next()) {
	...
	public static Map<String,Double> mapDctoGrupo (Connection con, String db){
		...
		String query = String.format(" select id_bodegaEmpresa, id_grupo, tasaDescto from `%s`.dctoGrupo where tasaDescto > 0;",db);
		try (PreparedStatement smt = con.prepareStatement(query)){
			try(ResultSet rs = smt.executeQuery()) {
				while (rs.next()) {
	...
	public static Map<String,Double> mapDctoEquipo (Connection con, String db){
		Map<String,Double> map = new HashMap<String,Double>();
		String query = String.format(" select id_bodegaEmpresa, id_equipo, tasaDescto from `"+db+"`.dctoEquipo where tasaDescto > 0;",db);
		try (PreparedStatement smt = con.prepareStatement(query)){
			try(ResultSet rs = smt.executeQuery()) {
				while (rs.next()) {
					map.put(rs.getString(1) + "_" + rs.getString(2), rs.getDouble(3));
