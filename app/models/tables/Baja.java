...
public class Baja {
	...
	private static final Logger logger = LoggerFactory.getLogger(Baja.class);
	...
	public Long getId_actaBaja() {
		return id_actaBaja;
	}

	public void setId_actaBaja(Long id_actaBaja) {
		this.id_actaBaja = id_actaBaja;
	}
	...
	public Long getId_movimientoOrigen() {
		return id_movimientoOrigen;
	}

	public void setId_movimientoOrigen(Long id_movimientoOrigen) {
		this.id_movimientoOrigen = id_movimientoOrigen;
	}
	...
	public String getFecha_actaBaja() {
		return fecha_actaBaja;
	}

	public void setFecha_actaBaja(String fecha_actaBaja) {
		this.fecha_actaBaja = fecha_actaBaja;
	}
	...
	public static Map<Long, Double> totalDeBajasPorIdEquipoPendientes(Connection con, String db) {
		...
		try (PreparedStatement smt = con.prepareStatement(
				"select id_equipo, sum(cantidad) from `" + db + "`.baja where fechaConfirma is null group by id_equipo;")) {
	...
	public static Map<Long, Double> totalDeBajasPorIdEquipo(Connection con, String db) {
		Map<Long, Double> aux = new HashMap<Long, Double>();
		try (PreparedStatement smt = con.prepareStatement(
				"select id_equipo, sum(cantidad) from `" + db + "`.baja group by id_equipo;")) {
	...
	public static boolean create(Connection con, String db, String detalle) {
		boolean flag = false;
		try (PreparedStatement smt1 = con.prepareStatement(
				"insert into `" + db + "`.baja (id_actaBaja, id_equipo, cantidad, motivo) values " + detalle + ";")) {
	...
	public static boolean deleteNoConfirmadas(Connection con, String db, Long id_actaBaja) {
		boolean flag = false;
		try (PreparedStatement smt = con.prepareStatement(
				" delete from `" + db + "`.baja where id_actaBaja = ? and esModificable=1 and fechaConfirma is null;")) {
	...
	public static List<List<String>> listaBajaPorEquipo(Connection con, String db) {
		List<List<String>> lista = new ArrayList<List<String>>();
		try (PreparedStatement smt = con.prepareStatement(
				"select baja.id_equipo, grupo.nombre, equipo.codigo, equipo.nombre, unidad.nombre, sum(baja.cantidad) "
						+ " from `" + db + "`.baja "
						+ " left join `" + db + "`.equipo on equipo.id = baja.id_equipo "
