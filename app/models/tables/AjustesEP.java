...
public class AjustesEP {
	...
	public static List<AjustesEP> allPorBodega(Connection con, String db, Long id_bodegaEmpresa, String esPorSucursal, String id_sucursal){
		...
		try {
			...
			while (rs.next()) {
				...
				String fecha = "";		
				if (rs.getString(6) != null) {fecha = myformatfecha.format(rs.getDate(6));}
				lista.add(new AjustesEP(rs.getLong(1),rs.getLong(2),rs.getLong(3),rs.getLong(4),rs.getString(5),fecha,rs.getLong(7),myformatdouble.format(rs.getDouble(8)),
						rs.getString(9),rs.getString(10),rs.getString(11),rs.getString(12),rs.getString(13),rs.getString(14),rs.getLong(15),nameSucursal));
			}
	...
	public static AjustesEP find(Connection con, String db, Long id_ajuste, String esPorSucursal, String id_sucursal){
		...
		try {
			...
			if (rs.next()) {
				...
				String fecha = "";		
				if (rs.getString(6) != null) {fecha = myformatfecha.format(rs.getDate(6));}
				aux = new AjustesEP(rs.getLong(1),rs.getLong(2),rs.getLong(3),rs.getLong(4),rs.getString(5),fecha,rs.getLong(7),myformatdouble.format(rs.getDouble(8)),
						rs.getString(9),rs.getString(10),rs.getString(11),rs.getString(12),rs.getString(13),rs.getString(14),rs.getLong(15),nameSucursal);
			}
	...
	public static boolean update(Connection con, String db, AjustesEP aux){
		...
		try {
			...
			PreparedStatement smt = con
					.prepareStatement("update `"+db+"`.ajustesEP set "+ 
	...
	public static boolean delete(Connection con, String db, Long idAjuste){
		...
		try {
			PreparedStatement smt = con
					.prepareStatement("delete from `"+db+"`.ajustesEP where id=?;");
	...
	public static boolean create(Connection con, String db, AjustesEP aux){
		...
		try {
			...
			PreparedStatement smt2 = con
					.prepareStatement("Select id from `"+db+"`.ajustesEP where "+ 
			...
			if(!rs2.next()) {
				PreparedStatement smt = con
						.prepareStatement("insert into `"+db+"`.ajustesEP "+ 
	...
	public static List<AjustesEP> allPorPeriodos(Connection con, String db, String desdeAAMMDD, String hastaAAMMDD, String permisoPorBodega, String esPorSucursal, String id_sucursal){
		List<AjustesEP> lista = new ArrayList<AjustesEP>();
